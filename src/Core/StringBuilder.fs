...
module StringBuilder=
    ...
    let inline appendStringIfValueIsNotNullOrEmpty value = appendIfTrue (Fake.Core.String.isNullOrEmpty value |> not)

    /// Appends a text if the value is not null or empty.
    let inline appendIfNotNullOrEmpty value s = appendIfTrue (Fake.Core.String.isNotNullOrEmpty value) (sprintf "%s%s" s value)

    /// Appends all notnull fileNames.
    let inline appendFileNamesIfNotNull fileNames (builder : StringBuilder) = 
        fileNames |> Seq.fold (fun builder file -> appendIfTrue (Fake.Core.String.isNullOrEmpty file |> not) file builder) builder
